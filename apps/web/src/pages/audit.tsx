    ...
    <article className="relative rounded-lg border border-border bg-background/30 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs font-semibold text-cyan-200">#{event.sequence}</span>
          <span className="text-xs text-muted-foreground">{formatDateTime(event.occurredAt)}</span>
        </div>
        <span className="rounded border border-border px-2 py-1 text-[0.65rem] uppercase tracking-[0.1em] text-muted-foreground">{humanizeStatus(event.eventType)}</span>
      </div>
      <p className="mt-3 break-words text-sm font-medium text-foreground">{event.summary}</p>
      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
        <span>Actor: <strong className="font-medium text-foreground">{event.actor}</strong></span>
        <span className="break-words">Entity: <strong className="break-all font-mono font-normal text-foreground">{event.entityId ?? "Global"}</strong></span>
      ...
      {event.toolTrace && (
        <details className="mt-3 border-t border-border/70 pt-3">
          <summary className="flex cursor-pointer list-none items-center gap-2 text-xs text-muted-foreground"><Wrench className="size-3.5" aria-hidden="true" /> Tool trace</summary>
...
export function AuditPage() {
  ...
  if (audit.isLoading) return <PageState kind="loading" headingLevel="h1" title="Loading audit" description="Reading outcome-focused chronology, actors, and tool traces." />;
  if (audit.isError) return <PageState kind="error" headingLevel="h1" title="Unable to load audit" description={audit.error.message} action={<RetryButton onClick={() => void audit.refetch()} />} />;
  if (!audit.data) return <PageState kind="error" headingLevel="h1" title="Audit data is unavailable" description="The API returned no audit payload." />;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-3 border-b border-border pb-6 sm:flex-row sm:items-end">
        <div><h1 className="text-3xl font-semibold tracking-tight">Audit</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">A chronological record of runs, evidence outcomes, advisory tools, and human decisions.</p></div>
      ...
      <section className="grid gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Audit filters">
        ...
        <label className="space-y-1.5 text-xs font-medium text-muted-foreground" htmlFor="audit-event-type">Event type<Select id="audit-event-type" value={filters.eventType ?? ""} onChange={(event) => updateFilter("eventType", event.target.value)}>{eventTypes.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></label>
      </section>
      <Card>
        <CardHeader className="flex-row items-center justify-between"><CardTitle className="text-sm font-medium">Event chronology</CardTitle>{audit.isFetching && !audit.isLoading && <span role="status" className="text-xs text-cyan-200">Updating audit...</span>}</CardHeader>
        <CardContent>
          {events.length === 0 ? <PageState kind="empty" title="No audit events match these filters." description="Outcome events will appear after a reset, run, investigation, or human review." /> : <div className="space-y-3">{events.map((event) => <AuditEventCard key={event.eventId} event={event} />)}<Pagination page={audit.data.page} total={audit.data.total} pageSize={pageSize} onPageChange={setPage} /></div>}
        </CardContent>
      </Card>
    </div>
  );

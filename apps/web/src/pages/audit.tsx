      ...
      <article className="border-b border-border pb-5 last:border-0">
        ...
        {event.toolTrace && (
          <details className="mt-3 border-t border-border/70 pt-3">
             <summary className="flex cursor-pointer list-none items-center gap-2 text-xs text-muted-foreground"><IconTool className="size-3.5" aria-hidden="true" /> Tool details</summary>
...
export function AuditPage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="Audit"
        description="A timeline of runs, evidence, AI checks, and review decisions."
      ...
      <Card className="gap-0 py-0">
        <CardHeader className="panel-header"><CardTitle className="text-sm font-medium">Event history</CardTitle>{audit.isFetching && !audit.isLoading && <span role="status" className="text-xs text-primary">Updating history…</span>}</CardHeader>
        <CardContent className={events.length === 0 ? "p-4" : "p-4 sm:p-5"}>
          {events.length === 0 ? <PageState kind="empty" title="No events match these filters." description="Events appear after a reset, run, investigation, or review." /> : <div><Timeline>{events.map((event) => <AuditEventCard key={event.eventId} event={event} />)}</Timeline><Pagination page={audit.data.page} total={audit.data.total} pageSize={pageSize} noun="events" onPageChange={setPage} /></div>}
        </CardContent>
      </Card>
    </div>
  );
